  ...
  return (
    <>
      {subStats.map(({ type, value }, i) => {
        ...
        return mutable ? (
          <div key={i} className="mt-2 flex items-center bg-dark-700 relative">
            <FaChevronDown className="absolute left-3 top-2.5" />

            <select
              className={
                "pt-2 pb-1 pr-2 pl-10 leading-base relative z-10 appearance-none " +
                (statTypeCount[type] === 1 ? "text-light-400" : "text-red-500")
              }
              value={type}
              onChange={(e) => {
            ...
            <Input
              ...
              className={
                ...
                (isValid ? "text-light-400" : "text-red-500")
              }
          ...
          <div key={i} className={`mt-2 pt-2 pb-1 flex items-center bg-dark-700`}>
            <p className={space}>•</p>
            <p>
              <span className={"mr-1 " + (statTypeCount[type] === 1 ? "text-light-400" : "text-red-500")}>{t(type)}</span>
